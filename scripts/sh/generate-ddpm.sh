...
#
# Usage:
#   bash scripts/sh/generate-ddpm.sh <checkpoint_path> [OPTIONS]
#
# Examples (DDPM — default, full 1000 steps):
#   bash scripts/sh/generate-ddpm.sh experiments/20260207-ddpm/checkpoints/checkpoint_final.pt
#   bash scripts/sh/generate-ddpm.sh path/to/checkpoint.pt --num_samples 25 --nrow 5
#
...
#   bash scripts/sh/generate-ddpm.sh path/to/checkpoint.pt --mode ddim --ddim_steps 100 --eta 0.5
#
...
if [ -z "$1" ]; then
    ...
    echo "Usage: bash scripts/sh/generate-ddpm.sh <checkpoint_path> [OPTIONS]"
    exit 1
fi
...
python -m scripts.python.generate_ddpm --checkpoint "$CHECKPOINT" "$@"
