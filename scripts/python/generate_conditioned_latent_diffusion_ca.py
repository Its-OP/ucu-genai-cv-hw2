...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate cross-attention class-conditioned samples from a "
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to trained cross-attention conditioned latent UNet "
        "checkpoint (.pt file)",
    )
    parser.add_argument(
        "--class_label",
...
def main():
    ...
    run_directory = os.path.join(
        args.output_dir,
        f"{timestamp}-conditioned-latent-ca-ddim",
    )
    ...
    # Verify checkpoint is a cross-attention conditioned model
    if not isinstance(model, CrossAttentionConditionedUNet):
        raise ValueError(
            "The provided UNet checkpoint is not a cross-attention conditioned "
            "model. Use generate_conditioned_latent_diffusion.py for "
            "channel-concat models, or generate_latent_diffusion.py for "
    ...
    print(f"Cross-attention dim: {conditioned_unet.context_dim}")
