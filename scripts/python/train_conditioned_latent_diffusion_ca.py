...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Cross-Attention Class-Conditioned Latent Diffusion "
        "Model on MNIST"
    )
    parser.add_argument(
        "--vae_checkpoint",
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Dimension of the dense class embedding for cross-attention "
        "(default: 128)",
    )
    parser.add_argument(
        "--guidance_scale",
...
def train_epoch(
    ...
    """Train the cross-attention conditioned latent-space UNet for one epoch.

    The diffusion training loss in latent space with cross-attention conditioning:
        L = E_{t, z_0, epsilon, c}[|| epsilon - epsilon_theta(z_t, t; c) ||^2]

    where z_0 = scaling_factor · Enc(x) is the scaled VAE-encoded input,
    and c is the class embedding context (randomly dropped with probability
...
def evaluate(
    ...
    """Evaluate cross-attention conditioned latent diffusion model on test set."""
...
def generate_conditioned_samples(
    ...
    for significantly faster generation.
    ...
    Args:
        conditioned_unet: Trained CrossAttentionConditionedUNet.
        ddpm: DDPM scheduler.
...
def main():
    ...
    exp_dir = setup_experiment_folder(
        f"./experiments/{timestamp}-conditioned-latent-diffusion-ca"
    )
    ...
    config = {
        ...
        # Cross-attention: input == output == latent_channels (no extra channel)
    ...
    # Initialize cross-attention class-conditioned latent-space UNet
    # UNet input: latent_channels (no extra channel — conditioning via cross-attention)
    # UNet output: latent_channels (noise prediction in latent space)
    conditioned_unet = create_cross_attention_conditioned_unet(
        latent_channels=latent_channels,
        number_of_classes=args.number_of_classes,
        context_dim=args.cross_attention_dim,
        unconditional_probability=args.unconditional_probability,
        base_channels=args.base_channels,
    ...
    print(f"Cross-Attention Conditioned Latent UNet parameters: {num_params:,}")
    print(
        f"  Cross-attention conditioning: {args.number_of_classes} classes, "
        f"context_dim={args.cross_attention_dim}, "
