...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Latent Rectified Flow Model on MNIST"
    )
    parser.add_argument(
        "--vae_checkpoint",
        type=str,
...
def encode_batch(
    ...
    to avoid injecting VAE reparameterization noise on top of the flow noise.

    The latents are scaled by ``scaling_factor`` (= 1/std(z)) so that the
    velocity prediction operates on data with approximately unit variance.
...
def compute_latent_scaling_factor(
    ...
    The Rectified Flow interpolation assumes data with approximately unit
    variance. VAE latents may have different variance, so we compute
    ``scaling_factor = 1 / std(z)`` to normalize them.
...
def train_epoch(
    unet, rectified_flow, vae, optimizer, loader, device,
    scaling_factor=1.0, scaler=None, ema=None,
):
    """Train the latent-space UNet for one epoch using Rectified Flow.

    The velocity loss in latent space is:
        L = E_{t, z_0, epsilon}[|| v_theta(z_t, t) - (epsilon - z_0) ||^2]
    ...
    for images, _ in tqdm(loader, desc="Training", leave=False):
        ...
        continuous_time = torch.rand(latent.shape[0], device=device) * (1.0 - 1e-5) + 1e-5

        optimizer.zero_grad()

        if use_amp:
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                loss = rectified_flow.velocity_loss(unet, latent, continuous_time)
...
@torch.no_grad()
def evaluate(unet, rectified_flow, vae, loader, device, scaling_factor=1.0, use_amp=False):
    """Evaluate latent Rectified Flow model on test set."""
    unet.eval()
    total_loss = 0.0
...
def generate_samples(
    unet, rectified_flow, vae, num_samples, latent_channels, device, scaling_factor=1.0,
):
    """
    Generate pixel-space samples via the latent Rectified Flow pipeline.

    Pipeline:
        1. Sample noise in latent space: z_1 ~ N(0, I), shape (B, latent_channels, 4, 4)
        2. Euler integration from t=1 to t=0: z_1 -> z_0_scaled
        3. Unscale latents: z_0 = z_0_scaled / scaling_factor
        4. Decode to pixel space: x_hat = Dec(z_0), shape (B, 1, 32, 32)
        5. Crop back to 28x28

    Args:
        unet: Trained latent-space UNet.
        rectified_flow: RectifiedFlow scheduler.
    ...
    """
    # Generate in latent space with clip_denoised=False (latent values are unbounded)
    latent_shape = (num_samples, latent_channels, 4, 4)
    latent_samples = rectified_flow.euler_sample_loop(unet, latent_shape, clip_denoised=False)

    # Unscale latents back to original VAE range
...
def main():
    ...
    exp_dir = setup_experiment_folder(f"./experiments/{timestamp}-latent-rf")
    ...
    # Model config for checkpoint reconstruction
    model_config = {
        "image_channels": latent_channels,
        "base_channels": args.base_channels,
        "channel_multipliers": list(channel_multipliers),
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = train_epoch(
            unet, rectified_flow, vae, optimizer, train_loader, device,
            scaling_factor=scaling_factor, scaler=scaler, ema=ema,
        )

        ema.apply_shadow(unet)
        eval_loss = evaluate(
            unet, rectified_flow, vae, test_loader, device,
        ...
        if (epoch + 1) % args.sample_every == 0:
            ...
            samples = generate_samples(
                unet, rectified_flow, vae, 10, latent_channels, device,
